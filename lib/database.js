...
class Database {
    ...
    async deleteRole (roleID){
        const toRemove = this.getRoleByID(roleID);
        if (toRemove.length < 1){
            console.error(`Role with ID ${roleID} does not exist and can not be deleted`);
            return;
        }
        ...
        //If results affect one or more rows - accounting for cascading deletions
        if (results.affectedRows > 0) {
            console.log(`The role ${toRemove[0].name} has been removed. Employees associated with this role have had their role set to NULL. Please update employees roles are your earliest convenience.`)
        }
        else {
    ...
    async deleteEmployee (empID) {
        const toDelete = this.getEmployeeByID(empID);
