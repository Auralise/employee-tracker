...
class Database {
    ...
    async deleteDepartment(deptID) {
        ...
        if (results.affectedRows > 0) {
            console.log(`The department ${toRemove[0].department_name} and all associated roles have been removed. Employees have had their role set to NULL. Please update employees roles are your earliest convenience.`)
        }
        else {
    ...
    async addRole(title, salary, department_id) {
        ...
        const params = [title.toLowerCase(), salary, department_id];
    ...
    async getEmployeesWithoutRole() {
        const conn = await mysql.createConnection(this.details);
        const sql = 'SELECT id, first_name, last_name FROM employee WHERE role_id = NULL;'
